...
fastify.post('/units', { preHandler: ensureRole(['Admin']) }, async (req, reply) => {
  ...
  if (error) {
    ...
    return reply.code(500).send({ error: 'internal_error', details: error.message || error });
  }
...
fastify.get('/dashboard/metrics', { preHandler: ensureRole(['Admin', 'Gerente', 'Financeiro']) }, async (req, reply) => {
  try {
    ...
    const totalUnidades = (units || []).filter(u => u.status === 'Ativa' || u.status === 'Active' || u.status === 'Ativo').length;
    ...
    for (let i = 4; i >= 0; i--) {
      ...
      const { data: repM, error: repMErr } = await supabase
        .from('repasses')
        .select('net_value,status,paid_at')
        .gte('paid_at', iso(start))
        .lte('paid_at', iso(end))
        .eq('status', 'Paga');
      if (repMErr) throw new Error(repMErr.message);
      const repassesVal = (repM || []).reduce((s, r) => s + Number(r.net_value || 0), 0);

      const despesas = (expM || []).reduce((s, r) => s + Number(r.amount || 0), 0) + repassesVal;
