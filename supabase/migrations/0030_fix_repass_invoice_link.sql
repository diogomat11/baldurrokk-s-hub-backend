-- 0030_fix_repass_invoice_link.sql

-- Update generate_repass_preview to link Invoices to Professionals via Student -> Class -> Teacher
...
BEGIN
  ...
  invoices_calc AS (
    SELECT 
      -- We need to group by Entity ID.
      -- Since an invoice might belong to a Unit AND a Professional, we need to handle both cases.
      -- To do this in a single query efficiently:
      te.id as entity_id,
...
-- Update confirm_repass to link Invoices to Professionals
